...
std::vector<PositionedBlock> Utils::GetNearbySolidBlocks(glm::ivec3 pos, int tallness) { //basically gathers the surroning blocks around a position
	...
	if (Game::overworld->IsSolidBlock(pos.x, pos.y - 1, pos.z))
		returnal.push_back({ Game::overworld->getBlockAt(pos.x, pos.y - 1, pos.z), glm::ivec3(pos.x, pos.y - 1, pos.z) });

	for (int i = 0; i < tallness; i++) {
		if (Game::overworld->IsSolidBlock(pos.x + 1, pos.y + i, pos.z))
			returnal.push_back({ Game::overworld->getBlockAt(pos.x + 1, pos.y + i, pos.z), glm::ivec3(pos.x + 1, pos.y + i, pos.z) });
		if (Game::overworld->IsSolidBlock(pos.x - 1, pos.y + i, pos.z))
			returnal.push_back({ Game::overworld->getBlockAt(pos.x - 1, pos.y + i, pos.z), glm::ivec3(pos.x - 1, pos.y + i, pos.z) });
		if (Game::overworld->IsSolidBlock(pos.x, pos.y + i, pos.z + 1))
			returnal.push_back({ Game::overworld->getBlockAt(pos.x, pos.y + i, pos.z + 1), glm::ivec3(pos.x, pos.y + i, pos.z + 1) });
		if (Game::overworld->IsSolidBlock(pos.x, pos.y + i, pos.z - 1))
			returnal.push_back({ Game::overworld->getBlockAt(pos.x, pos.y + i, pos.z - 1), glm::ivec3(pos.x, pos.y + i, pos.z - 1) });
	}

	if (Game::overworld->IsSolidBlock(pos.x, pos.y + tallness, pos.z))
		returnal.push_back({ Game::overworld->getBlockAt(pos.x, pos.y + tallness, pos.z), glm::ivec3(pos.x, pos.y + tallness, pos.z) });
...
RayHitReturnParams Utils::RayHitBlock(Ray& ray, float steps) {
	for (float t = 0; t < ray.RayDistance; t += steps) {
		...
		if (Game::overworld->IsSolidBlock(blockPos.x, blockPos.y, blockPos.z)) {
			Block* blockPoint = Game::overworld->getBlockAt(blockPos.x, blockPos.y, blockPos.z);
