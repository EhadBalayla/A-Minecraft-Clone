...
void Chunk::CreateMeshObjects() {
    ...
    //generate buffer objects for transparents
    glGenVertexArrays(1, &meshes.m_VAO4);
    glGenBuffers(1, &meshes.m_VBO4);
    glGenBuffers(1, &meshes.m_EBO4);
}
void Chunk::DeleteMeshObjects() {
    ...
    //delete buffer objects for transparents
    glDeleteVertexArrays(1, &meshes.m_VAO4);
    glDeleteBuffers(1, &meshes.m_VBO4);
    glDeleteBuffers(1, &meshes.m_EBO4);
}
...
void Chunk::ChunkUpload() {
    ...
    if (meshData.transparentVerticies.size() > 0) {
        ...
        HasTransparent = true;
        meshes.transparentCount = meshData.transparentIndicies.size();
    }
    else HasTransparent = false;

    meshData.opaqueVerticies.clear();
    meshData.opaqueIndicies.clear();

    meshData.plantVerticies.clear();
    meshData.plantIndicies.clear();
