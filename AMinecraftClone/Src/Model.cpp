...
Model* InitModels::InitializeCloudsPlane() {
    float cloudVertices[] = {
        // positions        // texcoords
    ...
    Model* cloudModel = new Model();

    cloudModel->verticiesCount = 6;

    glGenVertexArrays(1, &cloudModel->VAO);
    glGenBuffers(1, &cloudModel->VBO);

    glBindVertexArray(cloudModel->VAO);
    glBindBuffer(GL_ARRAY_BUFFER, cloudModel->VBO);
